...
class Farmbot:
    ...
    def waterPot(self, pot):
        ...
        while(pot.checkHumidity()):
            self.watering=True
            superLog("Watering pot %s"%pot.potNumber)
            ...
            superLog("Humidity after waering %s"%(pot.humidityActual))
        self.watering=False
...
def superLog(mensaje):
    with open("farmbotLogs.txt", 'r+') as f:
